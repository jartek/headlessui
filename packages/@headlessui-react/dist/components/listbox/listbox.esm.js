...
import React, { useCallback, useReducer, createRef, useMemo, createContext, useRef, useContext, Fragment } from 'react';
...
}, _reducers[ActionTypes.ChangeValue] = function (state, action) {
  return _extends({}, state, {
    value: action.value
  });
}, _reducers);
...
function Listbox(props) {
  ...
  var onChange = useCallback(function (changedOption) {
    var newValue = Array.isArray(value) ? value.includes(changedOption) ? value.filter(function (option) {
      return option !== changedOption;
    }) : [].concat(value, [changedOption]) : changedOption;
...
function Option(props) {
  ...
  var selected = Array.isArray(state.value) ? state.value.includes(value) : state.value === value;
  var bag = useRef({
    disabled: disabled,
    value: value
  });
